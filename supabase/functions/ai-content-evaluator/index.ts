...
- Content quality is extremely poor`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-5-2025-08-07', // Use latest flagship model
          messages: [
...
async function performFullValidation(content: string, title: string, targetWords: number): Promise<ValidationResult> {
  ...
  console.log('Starting comprehensive validation:', { wordCount, targetWords, titleLength: title.length });

  // 1. Check for repetitive/filler content using AI + pattern detection
  const repetitionCheck = await checkRepetitiveContentAdvanced(content);
  if (repetitionCheck.hasViolations) {
    result.isValid = false;
    result.errors.push("Content contains repetitive, filler, or spam text");
    result.hasRepetitiveContent = true;
    result.details!.repetitionDetails = repetitionCheck.details;
  }

  // 2. Check title-content relevance using AI
  const relevanceScore = await checkTitleRelevanceAdvanced(title, content);
  result.titleRelevanceScore = relevanceScore;
  if (relevanceScore < 70) {
    result.isValid = false;
    result.errors.push(`Content is not relevant to the declared title (${relevanceScore}% relevance, minimum 70%)`);
  }

  // 3. Check dictionary compliance (Oxford words)
  ...
  if (dictionaryResult.compliancePercentage < 95) {
    result.isValid = false;
    result.errors.push(`Dictionary compliance too low: ${dictionaryResult.compliancePercentage.toFixed(1)}% (minimum 95%). Invalid words detected.`);
  }

  // 4. Check spelling mistakes (advanced pattern detection)
  const spellCheckResult = await checkSpellingMistakesAdvanced(content);
  ...
  if (spellCheckResult.errorPercentage > 25) {
    result.isValid = false;
    result.errors.push(`Too many spelling errors: ${spellCheckResult.errorPercentage.toFixed(1)}% (maximum 25% allowed)`);
  }
